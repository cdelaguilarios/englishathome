    ...
    this.$el.on('click', '.time-slot', function () {
      ...
      if (!plugin.isSelecting()) {  // if we are not in selecting mode
        if (isSlotSelected($(this))) {
          ...
          plugin.$el.trigger('deselected.artsy.dayScheduleSelector', [getSelection(plugin, plugin.$el.find('.time-slot[data-day="' + day + '"]').filter("[data-selected='selected']").first(), plugin.$el.find('.time-slot[data-day="' + day + '"]').filter("[data-selected='selected']").last()), day]);
